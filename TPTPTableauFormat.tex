...
\title{The TPTP Format for Clausal Tableaux Proofs}
\titlerunning{TPTP Clausal Tableaux}
...
%--------------------------------------------------------------------------------------------------
\begin{abstract}
This paper describes the (new) TPTP format for recording clausal tableau proofs.
...
Section~\ref{Tableau} describes the new TPTP format for recording clausal tableau proofs, and 
...
%--------------------------------------------------------------------------------------------------
\section{The (new) TPTP Format for Clausal Tableau Proofs}
\label{Tableau}
...
Figure~\ref{TableauPicture} shows the clausal tableau for the clauses in 
Figure~\ref{ExampleClausification}.
Figure~\ref{ExampleTableau} shows the TPTP format for recording the tableau.
...
Note that the role of the annotated formula records it as a {\tt lemma}.
...
\begin{verbatim}
cnf(t4,plain,              p(c) | ~ q(c),
    inference(extension,[status(thm),parent(t2:2),bind(X,$fot(c))],[c3]) ).
\end{verbatim}
}
\noindent
notes that the variable \smalltt{X} in \smalltt{t2} is bound to the first-order term
\smalltt{c} (assuming variables have been renamed apart so that it is clear that the 
\smalltt{X} is in \smalltt{t2}).
...
This paper has described the new TPTP format for recording clausal tableau proofs. 
...
\begin{enumerate}
\item Easy reconstruction of the tableau - this is can be done directly from the \smalltt{parent}s.
...
\item Concise and simple enough for a natural representation of proofs - there is no redundant
      information or unnecessary detail, and the tableau can be directly reconstructed.
...
In the larger long-term picture, the format can be extended to record non-clausal 
