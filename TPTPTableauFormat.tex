...
(GPL) Version 3.\footnote{Download from \href{https://www.cl.cam.ac.uk/~sbh11/connect++.html}{$\mathtt{https}$\texttt{://}$\mathtt{www.cl.cam.ac.uk}$\texttt{/}\textasciitilde$\mathtt{sbh11}$\texttt{/}$\mathtt{connect}\texttt{++}\mathtt{.html}$}.}
\cpp\ implements most of the search methods and other heuristics developed for $\mathtt{leanCoP}$ versions 
2 and later, including restricted backtracking (and an alternative version of backtracking restriction for 
extensions), iterative deepening by path length, various approaches to start clause selection, definitional clause conversion, 
deterministic or random re-ordering, and regularity testing. 
...
these can be verified internally, or output to a file and checked independently by a short Prolog program. 
