...
\address[2]{University of Cambridge, United Kingdom}
...
\begin{abstract}
This paper describes the (new) TPTP format for writing clausal connection tableaux.
...
Existing TPTP World tools for verifying and viewing derivations have been extended to
verify and view tableaux in this format.
\end{abstract}
\begin{keywords}
  TPTP \sep
  Tableaux \sep
...
The TPTP World (see Section~\ref{TPTP}) has an established format for writing refutations and
Hilbert-style proofs~\cite{SS+06}, and has a tool for verifying proofs in that format (see
Section~\ref{Derivations}).
One early proposal for a TPTP style format for writing non-clausal tableaux never gained 
traction~\cite{OS10}.
A more recent proposal for a TPTP style format for clausal connection tableaux~\cite{OH23} provided 
some inspiration for the design of the format described in this paper.
This paper describes the (new) TPTP format for writing \emph{clausal connection tableaux}.
...
Section~\ref{Tableau} describes the TPTP format for writing clausal connection tableaux, and 
...
Section~\ref{SystemsTools} describes an ATP system that can output clausal connection tableaux in 
the TPTP format, and explains how the GDV and IDV tools can be used to verify and view such 
tableaux.
...
The TPTP World~\cite{Sut24} is the well-established infrastructure that supports research, 
...
The TPTP and TSTP libraries, with their categorizations, provide the basis for assigning a 
difficulty rating to each problem, according to the number of ATP systems that are able to solve 
it~\cite{SS01}.
...
Figure~\ref{ExampleProblem} shows an example first-order form problem.
...
\caption{An example first-order form problem in TPTP format}
...
Three primary requirements were observed for the design of the TPTP format for clausal connection 
tableaux:
\begin{enumerate}
...
\end{enumerate}
Additional beneficial features, adopted from~\cite{OH23}, are:
\begin{enumerate}[resume]
\item Concise and simple enough for a natural representation of proofs.
\item Readable by humans as well as ATP tools.
\end{enumerate}
One early proposal for a TPTP style format for non-clausal tableaux~\cite{OS10} met requirements~1
and~3, but fell short of requirement~2 and features~4 and~5 because it attempted to 
``squeeze''~\cite{OH23} connection proofs into a derivation style format, with a clumsy encoding 
of parent formulae details.
It also did not provide an explicit way of recording lemmas.
Another existing TPTP style format is the leanTPTP format for clausal connection tableaux that is 
output by the leanCoP ATP system.~\cite{OB03}
The leanTPTP format definitely meets requirement~4, but omits the information required for 
requirements~1,~2, and~3. 
Most notably, the leanTPTP format does not record the tableau tree structure, does not
explicitly close the tableau branches, and does not appear to have any way of recording lemmas.
A more recent proposal for a TPTP style format for clausal connection tableaux~\cite{OH23} has 
weaknesses akin to the leanTPTP format.
The (new) TPTP format for writing clausal connection tableaux meets all the requirements and
provides both the beneficial features.
...
\smalltt{q(Y)} of \smalltt{t1} to the 1st literal \smalltt{{\tildenot}q(Y)} of \smalltt{c2}.
...
between \smalltt{q(Y)} and \smalltt{{\tildenot}q(Y)} in the extension to \smalltt{t2}.
...
between the 2nd literal \smalltt{{\tildenot}q(Y)} of \smalltt{t6} and the 1st literal 
...
The lemma is the negation of the 2nd literal \smalltt{{\tildenot}p(c)} of \smalltt{t2}.
...
\smalltt{{\tildenot}p(c)} of \smalltt{t2}.
...
program. 
From version 0.7.0 \cpp\ can also produce closed tableau in the TPTP format.
...
The extension of GDV to verify tableau proofs requires extending the structural verification 
phase\footnote{%
This ongoing work that will be completed in June 2026}, and a minor change to inference 
verification.
...
This is shown in Figure~\ref{ITVTableau} with the cursor hovering over \smalltt{{\tildenot}q(c)}
in the tableau.
...
This paper has described the TPTP format for writing clausal connection tableaux. 
The format builds on the existing infrastructure of the TPTP World, in particular the TPTP 
format for writing derivations.
The format meets the requirements and beneficial features listed in Section~\ref{Tableau}:
\begin{enumerate}
...
\end{enumerate}
The format has been adopted in the \cpp\ ATP system, and the existing GDV and IDV tools for 
verifying and viewing derivations have been adapted to verifying and viewing tableaux.

Short-term future work includes completing structural verification in GDV. 
Medium term efforts will focus on convincing developers of ATP systems that produce clausal 
connection tableau to output their tableaux in the TPTP format.
In the long-term the format might be extended to record non-connection~\cite{FB+98,Hah01} and 
non-clausal~\cite{Ott11,Ott17} tableaux.
